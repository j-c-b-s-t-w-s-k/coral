...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing coral-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='coral-cli', help='coral-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Coral address to which the faucet should send')
...
parser.add_argument('coral_cli_args', nargs='*', help='Arguments to pass on to coral-cli (default: -signet)')

args = parser.parse_args()

if args.coral_cli_args == []:
    args.coral_cli_args = ['-signet']


def coral_cli(rpc_command_and_params):
    argv = [args.cmd] + args.coral_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = coral_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = coral_cli(['getnewaddress', 'faucet', 'bech32'])
