...
""" Interactive corald P2P network traffic monitor utilizing USDT and the
    net:inbound_message and net:outbound_message tracepoints. """

# This script demonstrates what USDT for Coral Core can enable. It uses BCC
...
def main(corald_path):
    peers = dict()

    corald_with_usdts = USDT(path=str(corald_path))

    # attaching the trace functions defined in the BPF program to the tracepoints
    corald_with_usdts.enable_probe(
        probe="inbound_message", fn_name="trace_inbound_message")
    corald_with_usdts.enable_probe(
        probe="outbound_message", fn_name="trace_outbound_message")
    bpf = BPF(text=program, usdt_contexts=[corald_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE:", sys.argv[0], "path/to/corald")
        exit()
    path = sys.argv[1]
    main(path)
