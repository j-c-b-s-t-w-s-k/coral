...
const char * const CORAL_CONF_FILENAME = "coral.conf";
const char * const CORAL_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //coral-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", CORAL_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Coral
    // macOS: ~/Library/Application Support/Coral
    // Unix-like: ~/.coral
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Coral";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Coral";
#else
    // Unix-like
    return pathRet / ".coral";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const fs::path conf_path = GetPathArg("-conf", CORAL_CONF_FILENAME);
