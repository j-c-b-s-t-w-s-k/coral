// Copyright (c) 2011-2021 The Coral Core developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <qt/coralgui.h>
...
const std::string CoralGUI::DEFAULT_UIPLATFORM =
...
CoralGUI::CoralGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &CoralGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &CoralGUI::showModalOverlay);
...
void CoralGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Coral address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and coral: URIs)"));
    ...
    marketAction = new QAction(platformStyle->SingleColorIcon(":/icons/address-book"), tr("Mar&ket"), this);
    marketAction->setStatusTip(tr("Browse and trade on the decentralized Coral marketplace"));
    marketAction->setToolTip(marketAction->statusTip());
    marketAction->setCheckable(true);
    marketAction->setShortcut(QKeySequence(QStringLiteral("Alt+6")));
    tabGroup->addAction(marketAction);

    miningAction = new QAction(platformStyle->SingleColorIcon(":/icons/mining"), tr("&Mining"), this);
    miningAction->setStatusTip(tr("Mine Coral blocks with CPU"));
    ...
    connect(overviewAction, &QAction::triggered, this, &CoralGUI::gotoOverviewPage);
    ...
    connect(marketAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    ...
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Coral Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a coral: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Coral command-line options").arg(PACKAGE_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &CoralGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &CoralGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &CoralGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &CoralGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &CoralGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &CoralGUI::setCurrentWallet);
                    activity->open(path);
                });
            }
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &CoralGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &CreateWalletActivity::created, this, &CoralGUI::setCurrentWallet);
            activity->create();
        });
        connect(m_close_all_wallets_action, &QAction::triggered, [this] {
            m_wallet_controller->closeAllWallets(this);
        });
        connect(m_mask_values_action, &QAction::toggled, this, &CoralGUI::setPrivacy);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &CoralGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &CoralGUI::showDebugWindow);
}

void CoralGUI::createMenuBar()
{
#ifdef Q_OS_MACOS
...
void CoralGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &CoralGUI::setCurrentWalletBySelectorIndex);
...
void CoralGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &CoralGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &CoralGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &CoralGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &CoralGUI::showProgress);
...
#ifdef ENABLE_WALLET
void CoralGUI::setWalletController(WalletController* wallet_controller)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &CoralGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &CoralGUI::removeWallet);

    auto activity = new LoadWalletsActivity(m_wallet_controller, this);
    activity->load();
}

WalletController* CoralGUI::getWalletController()
{
    return m_wallet_controller;
}

void CoralGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &CoralGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &CoralGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &CoralGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &CoralGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &CoralGUI::incomingTransaction);
    connect(this, &CoralGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
    wallet_view->setPrivacy(isPrivacyModeActivated());
    const QString display_name = walletModel->getDisplayName();
    m_wallet_selector->addItem(display_name, QVariant::fromValue(walletModel));
}

void CoralGUI::removeWallet(WalletModel* walletModel)
{
    if (!walletFrame) return;
...
void CoralGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame) return;
...
void CoralGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void CoralGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void CoralGUI::setWalletActionsEnabled(bool enabled)
{
...
void CoralGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &CoralGUI::toggleHidden);
...
void CoralGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void CoralGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void CoralGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void CoralGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void CoralGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void CoralGUI::openClicked()
{
...
void CoralGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
...
void CoralGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void CoralGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void CoralGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void CoralGUI::gotoVerifyMessageTab(QString addr)
{
...
void CoralGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Coral network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void CoralGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void CoralGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void CoralGUI::updateHeadersSyncProgressLabel()
{
...
void CoralGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void CoralGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &CoralGUI::quitRequested);
...
void CoralGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void CoralGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void CoralGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &CoralGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &CoralGUI::show);
                e->ignore();
            }
        }
    }
...
void CoralGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void CoralGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void CoralGUI::incomingTransaction(const QString& date, CoralUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(CoralUnits::formatWithUnit(unit, amount, true));
...
void CoralGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void CoralGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool CoralGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool CoralGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void CoralGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void CoralGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void CoralGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void CoralGUI::updateWindowTitle()
{
...
void CoralGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void CoralGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void CoralGUI::detectShutdown()
{
...
void CoralGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void CoralGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(CoralGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = CoralGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void CoralGUI::subscribeToCoreSignals()
{
...
void CoralGUI::unsubscribeFromCoreSignals()
{
...
bool CoralGUI::isPrivacyModeActivated() const
{
      ...
      m_platform_style{platformStyle}
{
    ...
    QList<CoralUnit> units = CoralUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const CoralUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, CoralUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const CoralUnit u : CoralUnits::availableUnits()) {
        menu->addAction(CoralUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(CoralUnit newUnits)
{
    setText(CoralUnits::longName(newUnits));
}
