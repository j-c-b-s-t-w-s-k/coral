...
/** Call node.updateRwSetting() with Coral 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const util::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Coral 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/coral/coral/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because coral 22.x will not try to read these.
...
//! Convert enabled/size values to coral -prune setting.
...
//! Get pruning enabled value to show in GUI from coral -prune setting.
...
//! Get pruning size value to show in GUI from coral -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayCoralUnit")) {
        settings.setValue("DisplayCoralUnit", QVariant::fromValue(CoralUnit::BTC));
    }
    QVariant unit = settings.value("DisplayCoralUnit");
    if (unit.canConvert<CoralUnit>()) {
        m_display_coral_unit = unit.value<CoralUnit>();
    } else {
        m_display_coral_unit = CoralUnit::BTC;
        settings.setValue("DisplayCoralUnit", QVariant::fromValue(m_display_coral_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if coral.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_coral_unit);
...
{
    if (new_unit.isNull() || new_unit.value<CoralUnit>() == m_display_coral_unit) return;
    m_display_coral_unit = new_unit.value<CoralUnit>();
    QSettings settings;
    settings.setValue("DisplayCoralUnit", QVariant::fromValue(m_display_coral_unit));
    Q_EMIT displayUnitChanged(m_display_coral_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/coral/coral/pull/8273
    ...
    // (https://github.com/coral-core/gui/issues/567).
