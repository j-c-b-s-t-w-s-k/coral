// Copyright (c) 2015-2021 The Coral Core developers
...
uint256 SendCoins(CWallet& wallet, SendCoinsDialog& sendCoinsDialog, const CTxDestination& address, CAmount amount, bool rbf)
{
    ...
    entry->findChild<CoralAmountField*>("payAmount")->setValue(amount);
...
void CompareBalance(WalletModel& walletModel, CAmount expected_balance, QLabel* balance_label_to_check)
{
    ...
    QString balanceComparison = CoralUnits::formatWithUnit(unit, expected_balance, false, CoralUnits::SeparatorStyle::ALWAYS);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_coral-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_coral-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_coral-qt  # macOS
void TestGUI(interfaces::Node& node)
{
    ...
    // Amount input
    CoralAmountField* amountInput = receiveCoinsDialog.findChild<CoralAmountField*>("reqAmount");
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'QT_QPA_PLATFORM=cocoa test_coral-qt' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI(m_node);
