...
#include <qt/coraladdressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Coral address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new CoralAddressEntryValidator(parent));
    widget->setCheckValidator(new CoralAddressCheckValidator(parent));
}
...
bool parseCoralURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no coral: URI
    if(!uri.isValid() || uri.scheme() != QString("coral"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!CoralUnits::parse(CoralUnit::BTC, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseCoralURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseCoralURI(uriInstance, out);
}

QString formatCoralURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("coral:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(CoralUnits::format(CoralUnit::BTC, info.amount, false, CoralUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openCoralConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetPathArg("-conf", CORAL_CONF_FILENAME));
    ...
    /* Open coral.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Coral.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Coral (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Coral (%s).lnk", chain));
}
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "coral.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("coral-%s.desktop", chain));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a coral.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Coral\n";
        else
            optionFile << strprintf("Name=Coral (%s)\n", chain);
