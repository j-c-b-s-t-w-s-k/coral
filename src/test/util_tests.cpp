...
BOOST_FIXTURE_TEST_CASE(util_ArgsMerge, ArgsMergeTestingSetup)
{
    ...
    //
    //   ARGS_MERGE_TEST_OUT=results.txt ./test_coral --run_test=util_tests/util_ArgsMerge
    //
...
BOOST_FIXTURE_TEST_CASE(util_ChainMerge, ChainMergeTestingSetup)
{
    ...
    // If check below fails, should manually dump the results with:
    //
    //   CHAIN_MERGE_TEST_OUT=results.txt ./test_coral --run_test=util_tests/util_ChainMerge
    //
    // And verify diff against previous results to make sure the changes are expected.
    //
    // Results file is formatted like:
...
BOOST_AUTO_TEST_CASE(test_LocaleIndependentAtoi)
{
    ...
    // Ensure legacy compatibility with previous versions of Coral Core's atoi64
    for (const auto& pair : atoi64_test_pairs) {
...
BOOST_AUTO_TEST_CASE(test_Capitalize)
{
    ...
    BOOST_CHECK_EQUAL(Capitalize("coral"), "Coral");
...
BOOST_AUTO_TEST_CASE(util_WriteBinaryFile)
{
    ...
    std::string expected_text = "coral";
