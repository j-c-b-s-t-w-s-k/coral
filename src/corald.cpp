...
static bool AppInit(NodeContext& node, int argc, char* argv[])
{
    ...
    // If Qt is used, parameters/coral.conf are parsed in qt/coral.cpp's main()
    ...
    if (HelpRequested(args) || args.IsArgSet("-version")) {
        ...
        if (args.IsArgSet("-version")) {
            ...
            strUsage += "\nUsage:  corald [options]                     Start " PACKAGE_NAME "\n"
                "\n";
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                return InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see corald -h for a list of options.\n", argv[i])));
            }
        }
        ...
        // -server defaults to true for corald but not for the GUI so do this here
...
MAIN_FUNCTION
{
    ...
    // Connect corald signal handlers
    noui_connect();
