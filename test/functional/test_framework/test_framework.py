#!/usr/bin/env python3
# Copyright (c) 2014-2021 The Coral Core developers
...
TMPDIR_PREFIX = "coral_func_test_"
...
class CoralTestMetaClass(type):
    """Metaclass for CoralTestFramework.

    Ensures that any attempt to register a subclass of `CoralTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'CoralTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("CoralTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("CoralTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class CoralTestFramework(metaclass=CoralTestMetaClass):
    """Base class for a coral test script.

    Individual coral test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave coralds and test.* datadir on exit or error")
        ...
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop coralds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use coral-cli instead of RPC for all commands")
    ...
    def setup(self):
        ...
        fname_corald = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "corald" + config["environment"]["EXEEXT"],
        )
        fname_coralcli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "coral-cli" + config["environment"]["EXEEXT"],
        )
        fname_coralutil = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "coral-util" + config["environment"]["EXEEXT"],
        )
        self.options.corald = os.getenv("CORALD", default=fname_corald)
        self.options.coralcli = os.getenv("CORALCLI", default=fname_coralcli)
        self.options.coralutil = os.getenv("CORALUTIL", default=fname_coralutil)
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: coralds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("CoralRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'corald', self.options.corald) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'coral-cli', self.options.coralcli) for v in versions]
        ...
        for i in range(num_nodes):
            ...
            if not test_node_i.version_is_at_least(170000):
                ...
                conf_file = test_node_i.coralconf
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a corald test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple corald test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as corald's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    corald=self.options.corald,
                    coral_cli=self.options.coralcli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in coral.conf
    ...
    def skip_if_no_corald_tracepoints(self):
        """Skip the running test if corald has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("corald has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_corald_zmq(self):
        """Skip the running test if corald has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("corald has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if coral-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("coral-wallet has not been compiled")

    def skip_if_no_coral_util(self):
        """Skip the running test if coral-util has not been compiled."""
        if not self.is_coral_util_compiled():
            raise SkipTest("coral-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if coral-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("coral-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether coral-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether coral-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_coral_util_compiled(self):
        """Checks whether coral-util was compiled."""
        return self.config["components"].getboolean("ENABLE_CORAL_UTIL")
