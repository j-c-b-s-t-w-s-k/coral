...
def wait_until_helper(predicate, *, attempts=float('inf'), timeout=float('inf'), lock=None, timeout_factor=1.0):
    ...
    from `CoralTestFramework` or `P2PInterface` class ensures the timeout is
...
def initialize_datadir(dirname, n, chain, disable_autoconnect=True):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    write_config(os.path.join(datadir, "coral.conf"), n=n, chain=chain, disable_autoconnect=disable_autoconnect)
...
def append_config(datadir, options):
    with open(os.path.join(datadir, "coral.conf"), 'a', encoding='utf8') as f:
...
def get_auth_cookie(datadir, chain):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "coral.conf")):
        with open(os.path.join(datadir, "coral.conf"), 'r', encoding='utf8') as f:
            for line in f:
