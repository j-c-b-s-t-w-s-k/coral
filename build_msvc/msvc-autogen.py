...
def parse_config_into_btc_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"Coral Core {version}\""

    with open(os.path.join(SOURCE_DIR,'../build_msvc/coral_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/coral_config.h'), "w", encoding="utf8") as btc_config:
...
def main():
    parser = argparse.ArgumentParser(description='Coral-core msbuild configuration initialiser.')
    ...
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/coral_config.h'), os.path.join(SOURCE_DIR, 'config/coral-config.h'))
